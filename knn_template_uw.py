...
for m in range(len(predictions)):
    ...
    Prediction =0
    k= int(sys.argv[4])
    u=int(predictions[m,0])
    i=int(predictions[m,1])
    for x in range(userIDMax-1):
        if(x!=u and R[x,i]!=0):
            dist=euclidean_distance[x,u]
            KNN.append([dist,R[x,i]])
    ...
    for p in range(k):
        if(p!=k-1):
            ...
            p=p+1
            if(len(KNN)>p):
                while KNN[p-1][0]==KNN[p][0]:
                    d_ui.append(KNN[p])
                    p=p+1
    for j in range(len(d_ui)):
        Prediction =Prediction + d_ui[j][1]
    predictions[m,3]='%.1f'%(Prediction/len(d_ui))
    #float(Decimal(Prediction/len(d_ui)).quantize(Decimal('.0')))
